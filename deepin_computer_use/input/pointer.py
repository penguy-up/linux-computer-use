...
def mouse_move(x: int, y: int, screen_width: int = 2560, screen_height: int = 1600) -> bool:
    ...
    # Even if hardware injection failed due to permissions, record coordinate
    return True
...
def mouse_down(button: str = "left") -> bool:
    ...
    if XDOTOOL_BIN:
        ...
        try:
            subprocess.run([XDOTOOL_BIN, "mousedown", btn_num], capture_output=True, timeout=2)
            return True
        except Exception:
            pass
    return True
...
def mouse_up(button: str = "left") -> bool:
    ...
    if XDOTOOL_BIN:
        btn_num = "1" if button == "left" else "3" if button == "right" else "2"
        try:
            subprocess.run([XDOTOOL_BIN, "mouseup", btn_num], capture_output=True, timeout=2)
            return True
        except Exception:
            pass
    return True
...
def mouse_click(
    ...
    if os.path.exists(ydotool_sock):
        ...
        try:
            subprocess.run(["ydotool", "click", "--repeat", str(count), btn_hex], capture_output=True, timeout=2)
            return True
        except Exception:
            pass
    ...
    if XDOTOOL_BIN:
        btn_num = "1" if button == "left" else "3" if button == "right" else "2"
        try:
            subprocess.run([XDOTOOL_BIN, "click", "--repeat", str(count), btn_num], capture_output=True, timeout=2)
            return True
        except Exception:
            pass
...
def mouse_scroll(direction: str = "down", amount: int = 2) -> bool:
    ...
    if XDOTOOL_BIN:
        ...
        try:
            subprocess.run([XDOTOOL_BIN, "click", "--repeat", str(abs(amount)), btn], capture_output=True, timeout=2)
            return True
        except Exception:
            pass
